...
async def create_item(payload: UserCreate, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):

    return await service.create(session, payload)


@router.put('/{item_id}', response_model=UserRead)
async def update_item(item_id: int, payload: UserUpdate, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
...
@router.post('/telegram/{telegram_id}', response_model=UserRead)
async def sync_telegram_user(telegram_id: int, username: str | None = None, full_name: str | None = None, session: AsyncSession = Depends(get_db)):
    return await service.get_or_create_telegram_user(session, telegram_id, username=username, full_name=full_name)
