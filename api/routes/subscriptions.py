...
from services.subscriptions import service
...
async def create_item(payload: SubscriptionCreate, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):

    return await service.create(session, payload)


@router.put('/{item_id}', response_model=SubscriptionRead)
async def update_item(item_id: int, payload: SubscriptionUpdate, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
...
async def user_subscriptions(user_id: int, session: AsyncSession = Depends(get_db)):
