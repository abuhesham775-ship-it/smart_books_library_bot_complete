...
from services.categories import service
...
async def create_item(payload: CategoryCreate, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):

    return await service.create(session, payload)


@router.put('/{item_id}', response_model=CategoryRead)
async def update_item(item_id: int, payload: CategoryUpdate, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
