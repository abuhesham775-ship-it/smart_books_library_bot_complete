...
@router.get('/{item_id}', response_model=ReviewRead)
...
async def create_item(payload: ReviewCreate, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):

    return await service.create(session, payload)


@router.put('/{item_id}', response_model=ReviewRead)
async def update_item(item_id: int, payload: ReviewUpdate, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
