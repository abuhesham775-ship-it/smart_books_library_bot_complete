...
from services.recommendations import service
...
@router.get('/{item_id}', response_model=RecommendationRead)
...
async def create_item(payload: RecommendationCreate, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):

    return await service.create(session, payload)


@router.put('/{item_id}', response_model=RecommendationRead)
async def update_item(item_id: int, payload: RecommendationCreate, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
...
async def user_recommendations(user_id: int, session: AsyncSession = Depends(get_db)):
    return await service.top_for_user(session, user_id)
