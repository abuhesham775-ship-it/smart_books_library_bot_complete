...
@router.get('/{item_id}', response_model=AnalyticsEventRead)
...
async def create_item(payload: AnalyticsEventCreate, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):

    return await service.create(session, payload)


@router.put('/{item_id}', response_model=AnalyticsEventRead)
async def update_item(item_id: int, payload: AnalyticsEventCreate, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
...
@router.post('/track/', response_model=AnalyticsEventRead)
async def track_event(payload: AnalyticsEventCreate, session: AsyncSession = Depends(get_db)):
    return await service.create(session, payload)
